...
Для тестирования функционала приложения можно использовать следующие учётные записи:

| Роль | Email | Пароль |
|:---:|:---:|:---:|
| 👑 Администратор | `app_admin@bookstore.com` | `ПО ЗАПРОСУ` |
| 👤 Обычный пользователь | `test@test.com` | `test123` |

> ⚠️ **Важно:** Эти учётные записи созданы специально для тестирования и не требуют реальной регистрации. Пароли указаны исключительно для локального тестирования функционала администратора и пользователя.

### Функционал по ролям

**👑 Администратор** имеет доступ к:
- Добавлению новых книг
- Редактированию и удалению существующих книг
- Управлению категориями (добавление, редактирование, активация/деактивация, удаление)
- Просмотру профиля с пометкой **"ADMIN"**

**👤 Обычный пользователь** может:
- Просматривать каталог книг
- Фильтровать книги по категориям
- Добавлять книги в избранное (локальное сохранение)
- Просматривать детальную информацию о книгах
